...
def adjust_gamma(image: torch.Tensor, gamma):
    ...
    gamma_tensor = gamma.view(-1, 1, 1, 1) * torch.ones_like(image)
...
def blend_light_source(input_scene, pred_scene, threshold=0.99, luminance_mode=False):
    binary_mask = (get_highlight_mask(input_scene, threshold=threshold, luminance_mode=luminance_mode) > 0.5)
    binary_mask = binary_mask.to("cpu", torch.bool)
